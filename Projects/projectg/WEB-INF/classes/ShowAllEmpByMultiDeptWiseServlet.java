...
//solution  way5  or way7
public class ShowAllEmpByMultiDeptWiseServlet extends GenericServlet{ 
  
 @Override  
 public void service(ServletRequest request,ServletResponse response)
	  throws ServletException,IOException{
	///get multiple dept ids send client
	
	//step1	  
	String deptIdsArr[]=request.getParameterValues("dept_no");
	
	//a)
	String temp="";// make string like   //  10,20,30
	
	if(deptIdsArr!=null && deptIdsArr.length>0){
	for(int p=1;p<=deptIdsArr.length;p++){
	    String value=deptIdsArr[p-1];
		temp =temp+value;
		if(p<deptIdsArr.length){
		 temp =temp+",";
		}	
	}
	...
	//b)
	//create where cluase for deptno in
	String temp2="";
	//create string like  //  where deptno in(10,20,30)
	if(deptIdsArr!=null && deptIdsArr.length>0){
		temp2 = temp2 +" where deptno in("+temp+")";
	}
    
	
	//C) //select * from emp where deptno in(10,20)
	String sql="select * from Emp "+ temp2;  // 
